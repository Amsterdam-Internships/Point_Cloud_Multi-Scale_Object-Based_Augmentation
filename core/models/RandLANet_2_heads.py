...
class Network:
    def __init__(self, dataset, config, resume=False, resume_path=None):
        ...
        with tf.variable_scope('layers'):
            self.logits_1, self.logits_2 = self.inference(self.inputs, self.is_training)

        #####################################################################
        # Ignore the invalid point (unlabeled) when calculating the loss #
        #####################################################################
        with tf.variable_scope('loss'):
            self.logits_1 = tf.reshape(self.logits_1, [-1, config.num_classes])
            self.logits_2 = tf.reshape(self.logits_2, [-1, config.num_classes])
            ...
            valid_logits_1 = tf.gather(self.logits_1, valid_idx, axis=0)
            valid_logits_2 = tf.gather(self.logits_2, valid_idx, axis=0)
            ...
            self.loss = self.get_loss(valid_logits_1, valid_logits_2, valid_labels, self.class_weights)
            # self.loss = self.get_iou_loss(valid_logits, valid_labels)
            # self.loss = self.get_focal_loss(valid_logits, valid_labels)
            # tf.Variable(initial_value=-tf.log(tf.reduce_sum(ious)), name='loss', trainable=True)
            # self.loss = tf.Variable(initial_value=self.get_iou_loss(valid_logits, valid_labels, self.class_weights), name='loss', trainable=True)
        ...
        with tf.variable_scope('results'):
            self.correct_prediction = tf.nn.in_top_k(valid_logits_1, valid_labels, 1)
            self.accuracy = tf.reduce_mean(tf.cast(self.correct_prediction, tf.float32))
            self.prob_logits = tf.nn.softmax(self.logits_1)
    ...
    def inference(self, inputs, is_training):
        ...
        f_layer_fc3_2 = helper_tf_util.conv2d(f_layer_drop_2, self.config.num_classes, [1, 1], 'fc_2', [1, 1], 'VALID', False,
                                            is_training, activation_fn=None)
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ops = [self.train_op,
                       ...
                       self.labels,
                       self.accuracy]
                _, _, summary, l_out, probs_1, probs_2, labels, acc = self.sess.run(ops, {self.is_training: True})
    ...
    def get_loss(self, logits_1, logits_2, labels, pre_cal_weights):
        # calculate the weighted cross entropy according to the inverse frequency
        one_hot_labels = tf.one_hot(labels, depth=self.config.num_classes)

        # MAJORITY HEAD LOSS
        pre_cal_weights_1 = pre_cal_weights
        for index in [4,5,6,8,9]:
            pre_cal_weights_1[0][index] = 0
        class_weights_1 = tf.convert_to_tensor(pre_cal_weights_1, dtype=tf.float32)
        weights_1 = tf.reduce_sum(class_weights_1 * one_hot_labels, axis=1)
        unweighted_losses = tf.nn.softmax_cross_entropy_with_logits(logits=logits_1, labels=one_hot_labels)
        weighted_losses_1 = unweighted_losses * weights_1
        output_loss_1 = tf.reduce_mean(weighted_losses_1)

        # MINORITY HEAD LOSS
        pre_cal_weights_2 = pre_cal_weights
        for index in [0,1,2,3,7]:
            pre_cal_weights_2[0][index] = 0
        class_weights_2 = tf.convert_to_tensor(pre_cal_weights_2, dtype=tf.float32)
        weights_2 = tf.reduce_sum(class_weights_2 * one_hot_labels, axis=1)
        unweighted_losses_2 = tf.nn.softmax_cross_entropy_with_logits(logits=logits_2, labels=one_hot_labels)
        weighted_losses_2 = unweighted_losses_2 * weights_2
        output_loss_2 = tf.reduce_mean(weighted_losses_2)

        output_loss = tf.div(tf.add(output_loss_1, output_loss_2), 2)
