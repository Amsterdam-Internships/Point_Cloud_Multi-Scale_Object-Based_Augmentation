...
class Network:
    def __init__(self, dataset, config, resume=False, resume_path=None):
        ...
        if self.config.saving:
            if self.config.saving_path is None:
                if not os.path.isdir('results/GANet/{}/'.format(additional_information)):
                    os.makedirs('results/GANet/{}/'.format(additional_information))
                if resume:
                    self.saving_path = join('results/GANet/{}/'.format(additional_information), sorted(listdir('results/GANet/'))[-1])
                else: 
                    self.saving_path = time.strftime('results/GANet/{}/Log_%Y-%m-%d_%H-%M-%S'.format(additional_information), time.gmtime())
            else:
        ...
        with tf.variable_scope('inputs'):
            ...
            if dataset.mode == 'evaluate_only':
                if not os.path.isdir('evaluate_logs/GANet/'):
                    os.makedirs('evaluate_logs/GANet/')
                self.Log_file = open('evaluate_logs/GANet/log_evaluate_GANet_' + additional_information + log_time + '.txt', 'w+')
            else:
                if not os.path.isdir('train_logs/GANet/'):
                    os.makedirs('train_logs/GANet/')
                self.Log_file = open('train_logs/GANet/log_train_GANet_' + additional_information + log_time + '.txt', 'w+')

        # 网络结构
        with tf.variable_scope('randlanet'):
            with tf.variable_scope('layers'):
                self.randla_feature = self.inference(self.inputs, self.is_training)
        ...
        if resume:
            if resume_path is None:
                ...
                resume_path = join('results/GANet/', sorted(listdir('results/GANet/'))[-1] + '/')
    ...
    def evaluate(self, dataset):
        ...
        log_out('mean recall: {}'.format(mean_recall), self.Log_file)
        ...
        log_out('Precision matrix', self.Log_file)
        s = '     '
        for class_name in class_names:
            s += '{}   '.format(str(class_name)[0:3])
        log_out(s, self.Log_file)
        log_out('-' * len(s), self.Log_file)
        s = ''
        for i in range(len(class_names)):
            s += '{}  '.format(str(class_names[i])[0:3])
            for j in range(len(conf_matrix[i])):
                value = conf_matrix[i][j] / sum(np.transpose(conf_matrix)[j])
                s += '{:0.3f} '.format(value)
            log_out(s, self.Log_file)
            s = ''
        log_out('\n', self.Log_file)
        ...
        # log recall matrix
        log_out('Recall matrix', self.Log_file)
        s = '     '
        for class_name in class_names:
            s += '{}   '.format(str(class_name)[0:3])
        log_out(s, self.Log_file)
        ...
        for i in range(len(class_names)):
            s += '{}  '.format(str(class_names[i])[0:3])
            for j in range(len(conf_matrix[i])):
                value = conf_matrix[i][j] / sum(np.transpose(conf_matrix)[i])
                s += '{:0.3f} '.format(value)
            log_out(s, self.Log_file)
            s = ''
        log_out('\n', self.Log_file)
